...
void init_particles()
{
    ...
    particle_data->num_particles = 10000;
    particle_data->particles = (Particle *)calloc(particle_data->num_particles, sizeof(Particle));
...
void start_particles()
{
    ...
    while(running)
    {
        ...
        while(particle_data->update_timer >= FRAME_TIME && update_count < MAX_UPDATES)
        {
            ...
            fprintf(particle_data->log_file, "render secs: %f\n", render_secs);
            ...
            update_count++;
            particle_data->update_timer -= FRAME_TIME;
...
static void render()
{
    ...
    {
        Particle *p = &(particle_data->particles[i]);
        render_quad(p->position, v2(1.0f, 1.0f) * p->scale);
    }
