...
struct RendererData
{
  ...
  std::vector<TrianglePrimitive> immediate_triangles_to_render;
  std::vector<LinePrimitive> immediate_lines_to_render;
...
{
  vertices[0] = Mesh::Vertex(v3(-1.0f, -1.0f, 0.0f), v3(0.0f, 0.0f, 0.0f), v2(0.0f, 1.0f)); // Left lower
  vertices[1] = Mesh::Vertex(v3( 1.0f, -1.0f, 0.0f), v3(0.0f, 0.0f, 0.0f), v2(1.0f, 1.0f)); // Right lower
  vertices[2] = Mesh::Vertex(v3( 1.0f,  1.0f, 0.0f), v3(0.0f, 0.0f, 0.0f), v2(1.0f, 0.0f)); // Right upper
  vertices[3] = Mesh::Vertex(v3(-1.0f,  1.0f, 0.0f), v3(0.0f, 0.0f, 0.0f), v2(0.0f, 0.0f)); // Left upper
...
void init_renderer(HWND window_handle, unsigned in_framebuffer_width, unsigned in_framebuffer_height, bool is_fullscreen, bool is_vsync)
{
  ...
  {
    renderer_data->triangle_mesh.vertices.resize(3);
    renderer_data->triangle_mesh.indices.resize(3);
    renderer_data->triangle_mesh.indices[0] = 0;
    renderer_data->triangle_mesh.indices[1] = 1;
    renderer_data->triangle_mesh.indices[2] = 2;
    renderer_data->triangle_mesh.fill_buffers(device);
    ...
    renderer_data->line_mesh.fill_buffers(device);
  }
}
...
static void render_scene(Camera *camera)
{
  ...
  for(unsigned i = 0; i < renderer_data->immediate_triangles_to_render.size(); i++)
  {
    TrianglePrimitive *current = &renderer_data->immediate_triangles_to_render[i];
    Mesh::Vertex vertices[3] = {};
    vertices[0].position = current->p;
    vertices[1].position = current->q;
    vertices[2].position = current->r;
    current->mesh->update_data(renderer_data->resources.device_context, &vertices, sizeof(vertices));
    
    v4 c = {current->color.r, current->color.g, current->color.b, current->color.a};
    render_mesh(current->mesh, camera, &renderer_data->quad_shader, v3(), v2(1.0f, 1.0f), 0.0f,
                c, 0, D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
  }

  for(unsigned i = 0; i < renderer_data->immediate_lines_to_render.size(); i++)
  {
    LinePrimitive *current = &renderer_data->immediate_lines_to_render[i];
    Mesh::Vertex vertices[2] = {};
    vertices[0].position = current->a;
    vertices[1].position = current->b;
    current->mesh->update_data(renderer_data->resources.device_context, &vertices, sizeof(vertices));
    
    v4 c = {current->color.r, current->color.g, current->color.b, current->color.a};
    render_mesh(current->mesh, camera, &renderer_data->quad_shader, v3(), v2(1.0f, 1.0f), 0.0f,
                c, 0, D3D_PRIMITIVE_TOPOLOGY_LINELIST);
  }


  renderer_data->immediate_triangles_to_render.clear();
  renderer_data->immediate_lines_to_render.clear();
}
...
ModelHandle create_model(v3 *vertices, unsigned num_vertices, unsigned *indices, unsigned num_indices, Color color)
{
  Model model;
  model.mesh = new Mesh();

  model.mesh->vertices.resize(num_vertices);
  model.mesh->indices.resize(num_indices);

  for(unsigned i = 0; i < num_vertices; i++)
  {
    Mesh::Vertex v;
    v.position = vertices[i];
    model.mesh->vertices[i] = v;
  }

  for(unsigned i = 0; i < num_indices; i++)  model.mesh->indices[i] = indices[i];
  ...
  renderer_data->models_to_render.push_back(model);
  ModelHandle handle = renderer_data->models_to_render.size() - 1;
...
{
  TrianglePrimitive t;
  t.mesh = &renderer_data->triangle_mesh;
  t.p = p;
  t.q = q;
  t.r = r;
  t.color = color;
  renderer_data->immediate_triangles_to_render.push_back(t);
}

void immediate_line(v3 a, v3 b, float thickness, Color color)
{
  LinePrimitive l;
  l.mesh = &renderer_data->line_mesh;
  l.a = a;
  l.b = b;
  l.color = color;
  renderer_data->immediate_lines_to_render.push_back(l);
}
